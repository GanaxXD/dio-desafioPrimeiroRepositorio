...
public class Main {
	...
	public static void main(String[] args) {
		List<String> aprovados = Arrays.asList("Ana", "Adriana", "Lucas", "Pedro", "Henrique", "Filó", "Hilton",
				"Wellington", "Jorge", "Maria", "Dimas");
		Predicate<String> letra = n -> n.contains("a");
		Function<String, String> contemH = n -> n.contains("H") ? (n + " contém a letra H") : (n + " não contém H");
		Function<String, String> contemE = n -> n.contains("e") ? (n + " e contém a letra E")
				: "E nem contém a letra E";
		...
		aprovados.stream().filter(letra).forEach(System.out::println);

		System.out.println("-------MAP-------\n");
		// Posso encadear maps
		aprovados.stream().map(contemH).map(contemE).forEach(System.out::println);
