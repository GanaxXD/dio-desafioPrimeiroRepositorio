...
public class Main {
	...
	public static void main(String[] args) {
		...
		// Function | S (um argumento) -> T (ou seja, recebe de um tipo e pode retornar
		// outro tipo)
		// Predicate | T -> boolean
		...
		System.out.println(parOuImpar.andThen(oResultado1).andThen(oResultado2).andThen(oResultado3).apply(32));
		...
		System.out.println(ehPar.and(temTresDigitos).test(900));
		System.out.println(ehPar.and(temTresDigitos).negate().test(900));

		System.out.println(ehPar.or(temTresDigitos).test(1200));
		...
		System.out.println(maisDez.andThen(vezezDois).andThen(aoQuadrado).apply(10));
		...
		System.out.println(aoQuadrado.compose(vezezDois).compose(maisDez).apply(10));

		System.out.println("\n\nBINARY OPERATOR" + "\n");
		// Recebe dois parâmetros e deolve um
		BinaryOperator<Double> mediaNotas = (num1, num2) -> (num1 + num2) / 2;
		System.out.println(mediaNotas.apply(7.5, 8.99));
